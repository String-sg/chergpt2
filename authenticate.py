		...
		if submit_button:
			...
			if user:
				...
				if user["pass_key"] == hashed_password:
					...
					st.session_state.bot_key = user.get("bot_key", st.session_state.bot_key)
					st.session_state.engine_key = user.get("engine_key", st.session_state.engine_key )
...
def class_login():
	with st.form(key='access'):
		...
		if submit_button:
			query_conditions = [{"vta_code" + str(i): vta_code} for i in range(1, 46)]
			query = {"$or": query_conditions}
			projection = {"_id": 0, "api_key": 1, "tch_code": 1,  "bot_key": 1, "engine_key": 1}
			cursor = user_info_collection.find(query, projection)
			for doc in cursor:
				if "api_key" in doc:
					result = doc["api_key"]
					st.session_state.bot_key = doc.get("bot_key", st.session_state.bot_key)
					st.session_state.engine_key =  doc.get("engine_key", st.session_state.engine_key )
