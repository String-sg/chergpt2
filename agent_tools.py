# This section imports the necessary libraries for the code to function
...
from langchain.agents import tool  # Custom tool for language chains
# Custom chains for language retrieval
from langchain.chains import RetrievalQA, RetrievalQAWithSourcesChain
# Custom embeddings from OpenAI
from langchain.embeddings.openai import OpenAIEmbeddings
# Custom chat models based on OpenAI
from langchain.chat_models import ChatOpenAI
from langchain import LLMChain  # Custom LLM chain
from langchain.vectorstores import Chroma, FAISS  # Custom vector storage methods
from typing import List, Dict  # Data types for type hinting
import requests  # Library to make HTTP requests
import json  # JSON library
import configparser  # To read configuration files
    ...
    try:
        ...
        for i, result in enumerate(search_results):
            try:
                ...
                if i == 0:
                    st.session_state.web_link.append(
                        f'Ref No: {st.session_state.s_count} - {result}')
                ...
                st.warning(
                    f"Unable to find Wikipedia resources for {result}: {str(e)}")
...
def extract_organic_results_and_people_also_ask(response_data: dict) -> dict:
    ...
    st.session_state.web_link.append(
        f'Ref No: {st.session_state.s_count} - {title}')
...
def extract_bing_results(response_data: dict) -> dict:
    ...
    related_searches = response_data.get(
        'relatedSearches', {}).get('value', [])
...
def document_search(query: str) -> str:
    ...
    if source_documents:
        ...
        st.session_state.web_link.append(
            f"Ref No: ({st.session_state.s_count})-{f_source}: {f_topic}")
    ...
    if source_documents:
        for document in source_documents:
            ...
            st.session_state.related_questions.append(
                f"Ref No: ({st.session_state.s_count})-{source}: {topic}, Content page {int(page) + 1}")
